...
import { ETH_DECIMALS, USDC_DECIMALS, USDQ_DECIMALS } from '../constants/contract';
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-20 pb-20">
      <div class="container mx-auto px-4 max-w-6xl">
            ...
            <div>
              ...
              <p class="text-xl font-semibold text-white">
                {{ web3Service.isConnected()
                  ? web3Service.getShortAddress(web3Service.connectedAccount() || '')
                  : 'Not Connected' }}
              </p>
            </div>
            <button
        ...
        @if (web3Service.errorMessage()) {
          <div class="mb-8 p-4 bg-red-900 border border-red-700 rounded-lg text-red-200">
        ...
        @if (web3Service.isConnected()) {
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <!-- USDC Balance -->
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-8 border border-slate-700 hover:border-cyan-500 transition-colors">
              ...
              <p class="text-3xl font-bold text-white mb-2">{{ formatBalance(usdcBalance()) }}</p>
              <p class="text-sm text-slate-400">USD Coin</p>
            </div>

            <!-- ETH Balance -->
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-8 border border-slate-700 hover:border-purple-500 transition-colors">
              <div class="flex items-center justify-between mb-4">
              ...
              <p class="text-3xl font-bold text-white mb-2">{{ formatBalance(ethBalance(), ETH_DECIMALS) }}</p>
              <p class="text-sm text-slate-400">Ethereum</p>
            </div>

            <!-- Wallet Status -->
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-8 border border-slate-700 hover:border-green-500 transition-colors">
              <div class="flex items-center justify-between mb-4">
              ...
              <p class="text-3xl font-bold text-white mb-2">{{ walletExists() ? 'Active' : 'Inactive' }}</p>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            @if (!walletExists()) {
              ...
              >
                {{ web3Service.isLoading() ? 'Creating Wallet...' : 'Create Virtual Wallet' }}
              </button>
            }

            <!-- Deposit / Withdraw Card -->
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 class="text-lg font-semibold text-white mb-4">Deposit & Withdraw</h3>
              <div class="flex flex-col gap-3">
                <button
                  (click)="activeTab.set('deposit')"
                  [ngClass]="{
                    'bg-gradient-to-r from-cyan-500 to-blue-500': activeTab() === 'deposit',
                    'bg-slate-700 hover:bg-slate-600': activeTab() !== 'deposit'
                  }"
                ...
                <button
                  ...
                  [ngClass]="{
                    'bg-gradient-to-r from-orange-500 to-red-500': activeTab() === 'withdraw',
                    'bg-slate-700 hover:bg-slate-600': activeTab() !== 'withdraw'
                  }"
                  class="w-full py-3 rounded-lg font-medium text-white transition-colors"
                >
                  Withdraw
            ...
            <!-- Convert Card -->
            <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700">
              <h3 class="text-lg font-semibold text-white mb-4">Convert Tokens</h3>
              <div class="flex flex-col gap-3">
                <button
                  (click)="activeTab.set('convert')"
                  [ngClass]="{
                    'bg-gradient-to-r from-purple-500 to-pink-500': activeTab() === 'convert',
                    'bg-slate-700 hover:bg-slate-600': activeTab() !== 'convert'
                  }"
                  class="w-full py-3 rounded-lg font-medium text-white transition-colors"
                >
                  Exchange Assets
          ...
          <div class="mt-12 p-6 bg-slate-800 rounded-lg border border-slate-700">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                <p class="text-cyan-400 font-mono text-xs break-all">0x797ADa8...</p>
              </div>
              <div>
          ...
          <div class="text-center py-20">
            <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-cyan-500 to-blue-500 rounded-full mb-6">
              <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
              </svg>
            </div>
            <h2 class="text-2xl font-bold text-white mb-2">Connect Your Wallet</h2>
            <p class="text-slate-400 mb-8">Connect MetaMask or another Web3 wallet to get started</p>
            <button
              (click)="toggleWallet()"
              [disabled]="web3Service.isLoading()"
